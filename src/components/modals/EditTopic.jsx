...
import { createTopic, updateTopic } from '../../http/topicApi'
import { Context } from '../../index'

const EditTopic = ({ show, onHide, topic }) => {
  const { user } = useContext(Context)
  console.log('данные пользователя :', user)
  console.log('данные Топика :', topic)
  ...
  const editTopicInfo = () => {
    ...
    const result = updateTopic(formData).then((res) => {
      if (res.statusText == 'OK') {
        alert('Топик успешно отредактирован')
        onHide()
      }
    })
  }
  ...
  return (
    ...
    >
      ...
      <Modal.Footer>
        <Button variant='outline-warning' onClick={editTopicInfo}>
          Заменить
        </Button>
